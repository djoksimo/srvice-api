  ...
  testCreateService() {
    ...
    describe("#ServiceManager.createService(payload)", () => {
      it("Should create a service and return id", async() => {
        ...
        assert.equal(201,res.status,"Fail: Status code should be 201");
      ...
      it ("Should fail creating the service and return error", async () => {
        ...
        assert.equal(500,res.status,"Fail: Status code should be 500");
        assert.equal("ValidationError",res.json.name,"Fail: Json should return ValidationError");
