...
router.post("/auth", (req, res) => isAuthenticated(req, res, async () => {
  HttpUtils.sendResponse(res, await chatManager.authenticateUser(req));
}));

router.post("/user", (req, res) => isAuthenticated(req, res, async () => {
  HttpUtils.sendResponse(res, await chatManager.createChatUser(req.body));
}));

router.post("/start", (req, res) => isAuthenticated(req, res, async () => {
  HttpUtils.sendResponse(res, await chatManager.startConversation(req.body));
}));

router.patch("/update-user", (req, res) => isAuthenticated(req, res, async () => {
